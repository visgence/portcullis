'''
' portcullis/management/commands/rebuild.py
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if settings.DATABASES['default']['NAME'].rpartition('/')[2] != 'portcullis.db':
            print 'It appears you are using custom database settings.  This command only works for portcullis.db.'
            sys.exit(1)
        ...
        try:
            os.remove(baseDir + "/portcullis.db")
        except OSError:
            print('portcullis.db does not exist.  Creating new db.')
